  ...
  const handleSubmit = (e: React.FormEvent, contentType: string) => {
    ...
    if (contentType === "여행지") {
      addDestination({
        ...
        highlights: (formData.get('dest-highlights') as string)?.split('\n').map(h => h.trim()).filter(Boolean),
        transportation: formData.get('dest-transportation') as string,
        accommodation: formData.get('dest-accommodation') as string,
        currency: formData.get('dest-currency') as string,
        language: formData.get('dest-language') as string,
        attractions: (formData.get('dest-attractions') as string)?.split('\n').map(a => {
          const [name, description = '', time = '', rating = ''] = a.split('|').map(s => s.trim());
          return name ? { name, description, time, rating: rating ? Number(rating) : undefined } : null;
        }).filter(Boolean),
        dailyBudget: {
          ...
          activity: formData.get('dest-budget-activity') as string,
        }
      });
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={(e) => handleSubmit(e, "여행지")} className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="dest-tips">여행 팁 (한 줄에 하나씩)</Label>
                    <Textarea id="dest-tips" name="dest-tips" placeholder="예: 렌터카 예약은 미리 하는 것이 좋습니다" className="mb-4" />
                  </div>
                  <div className="mb-4">
                    <Label htmlFor="dest-highlights">하이라이트 (한 줄에 하나씩)</Label>
                    <Textarea id="dest-highlights" name="dest-highlights" placeholder="예: 한라산\n성산일출봉" className="mb-4" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      <Label htmlFor="dest-transportation">교통 정보</Label>
                      <Input id="dest-transportation" name="dest-transportation" placeholder="예: 김포공항에서 1시간 30분" />
                    </div>
                    <div>
                      <Label htmlFor="dest-accommodation">숙박 정보</Label>
                      <Input id="dest-accommodation" name="dest-accommodation" placeholder="예: 리조트, 펜션, 게스트하우스 다양" />
                    </div>
                    <div>
                      <Label htmlFor="dest-currency">통화</Label>
                      <Input id="dest-currency" name="dest-currency" placeholder="예: 원화(KRW)" />
                    </div>
                    <div>
                      <Label htmlFor="dest-language">언어</Label>
                      <Input id="dest-language" name="dest-language" placeholder="예: 한국어" />
                    </div>
                  </div>
                  <div className="mb-4">
                    <Label htmlFor="dest-attractions">명소 (각 줄마다 이름|설명|시간|평점, 예: 한라산|한국 최고봉|6-8시간|4.9)</Label>
                    <Textarea id="dest-attractions" name="dest-attractions" placeholder="예: 한라산|한국 최고봉|6-8시간|4.9\n성산일출봉|유네스코 세계자연유산|2시간|4.8" className="mb-4" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <div>
                      <Label htmlFor="dest-budget-accommodation">숙박</Label>
                      <Input id="dest-budget-accommodation" name="dest-budget-accommodation" placeholder="예: 50,000~150,000원" />
                    </div>
                    <div>
                      <Label htmlFor="dest-budget-food">식비</Label>
                      <Input id="dest-budget-food" name="dest-budget-food" placeholder="예: 30,000~50,000원" />
                    </div>
                    <div>
                      <Label htmlFor="dest-budget-transport">교통</Label>
                      <Input id="dest-budget-transport" name="dest-budget-transport" placeholder="예: 40,000~60,000원" />
                    </div>
                    <div>
                      <Label htmlFor="dest-budget-activity">활동비</Label>
                      <Input id="dest-budget-activity" name="dest-budget-activity" placeholder="예: 20,000~80,000원" />
                    </div>
                  </div>
